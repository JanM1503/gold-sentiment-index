...
def generate_dashboard() -> None:
    ...
    if result:
        latest_gsi = float(result.get("gsi", 50.0))
        latest_class = str(result.get("classification", "Neutral"))
        latest_ts = str(result.get("timestamp", ""))
    else:
        latest_gsi = 50.0
        latest_class = "Neutral"
        latest_ts = ""

    # Gauge-style + time-series HTML using Chart.js
    html = f"""<!DOCTYPE html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\" />
...
<body>
  ...
  <script>
    const latestGsi = {latest_gsi};
    const latestTsRaw = {json.dumps(latest_ts)};

    // Format timestamp for display
    let latestTsDisplay = latestTsRaw;
    try {{
      if (latestTsRaw) {{
        const d = new Date(latestTsRaw);
        latestTsDisplay = d.toLocaleString('en-US', {{
    ...
    if (updatedEl) {{
      updatedEl.textContent = 'Last updated: ' + (latestTsDisplay || 'n/a');
    }}
    ...
    function drawGauge(value) {{
      ...
      drawBand(0, 25,  '#fee2e2');  // light red
      drawBand(25, 45, '#fed7aa');  // orange
